...
let tvQuery = `https://api.themoviedb.org/3/genre/tv/list?api_key=${apiKey}&language=en-US`;
...
let configquery =
  ...
  let popularQuery = `https://api.themoviedb.org/3/tv/popular?api_key=${apiKey}&language=en-US&page=1`;
...
const generateShows = (showslist, genre) => {
  ...
  for (const key in showslist) {
    ...
    let path = showslist[key]["poster_path"];
    let title = showslist[key]["original_name"];
    console.log('showslist[key]',showslist[key]);
    let overview = showslist[key]["overview"];
    imgurl = JSON.parse(imgurl);
    backurl = JSON.parse(backurl);
    backurl += showslist[key]["backdrop_path"];
    imgurl += path;
...
const generateGenres = async (data) => {
  for (let i = 0; i < 6; i++) {
    ...
    let tvQUeries = `
https://api.themoviedb.org/3/discover/tv?api_key=d7692980fe79006a4e3b17940a2cfd65&language=en-US&with_genres=${data.genres[i].id}`;
    let shows = await getData(tvQUeries);
    console.log(shows);
   // console.log(movies.results);
    generateShows(shows.results, data.genres[i].name);
  }
